...
A form that has a starting page which displays a list of tasks that can be grouped into sections. 
Each task is displayed with a status to indicate the progress the user has made through that task.
Users can click on a task to launch the associated set of pages which will finish with a **Check your answers** 
screen.
