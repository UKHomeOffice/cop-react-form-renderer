...
* #### `group: [object]`
Group is an array, which has `id, label` and `components`. Components is an array of strings. `id` should be unique for each group.
  `"groups:[{"id": "address","label": "Address details","components":["firstLineOfTheAddress","town","city","postCode"]}]"`, 
