...
should you want the opposite - i.e., **show** by default **show** - you should use the
...
the [Equality](#equality) operators.
...
Most scenarios would call for the [`in`](#within-array-any-one-of) operator, rather than the
`nin` one, but this is a useful option if there are a large number of items for which you would
want it to show but a much smaller and more manageable number for which you would want it hidden.
